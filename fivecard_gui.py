...
class PokerGame:
    ...
    def create_widgets(self):
        ...
        info_frame.pack(pady=10, expand=True, fill=tk.BOTH)
        info_frame.columnconfigure(0, weight=1)
        info_frame.columnconfigure(1, weight=1)
        info_frame.columnconfigure(2, weight=1)
        self.player_chips_label = tk.Label(info_frame, text=f"Your chips: {self.player_chips}", font=("Arial", 12))
        self.player_chips_label.grid(row=0, column=0, padx=20, sticky="ew")
        self.pot_label = tk.Label(info_frame, text=f"Pot: {self.pot}", font=("Arial", 12))
        self.pot_label.grid(row=0, column=1, padx=20, sticky="ew")
        self.computer_chips_label = tk.Label(info_frame, text=f"Computer chips: {self.computer_chips}", font=("Arial", 12))
        self.computer_chips_label.grid(row=0, column=2, padx=20, sticky="ew")

        #player hand
        self.player_frame = tk.Frame(self.root)
        self.player_frame.pack(pady=20, expand=True, fill=tk.BOTH)
        self.card_buttons = []
        for i in range(5):
            self.player_frame.columnconfigure(i, weight=1)
        #frame for buttons
        control_frame = tk.Frame(self.root)
        control_frame.pack(pady=5, expand=True, fill=tk.BOTH)
        ...
        self.computer_frame.pack(pady=20, expand=True, fill=tk.BOTH)
        for i in range(5):
            self.computer_frame.columnconfigure(i, weight=1)
    ...
    def start_new_hand(self):
        ...
        self.deck = create_deck()
        random.shuffle(self.deck)
        self.player_hand = [self.reshuffle_draw() for _ in range(5)]
        self.computer_hand = [self.reshuffle_draw() for _ in range(5)]
        self.selected.clear()
    ...
    def draw_cards(self):
        ...
        for idx in sorted(self.selected, reverse=True):
            self.discard_pile.append(self.player_hand.pop(idx))
        while len(self.player_hand) < 5:
            self.player_hand.append(self.reshuffle_draw())
        self.show_player_hand()

        #computer discards then draws
        comp_discard = computer_discard(self.computer_hand)
        for idx in sorted(comp_discard, reverse=True):
            self.discard_pile.append(self.computer_hand.pop(idx))
        while len(self.computer_hand) < 5:
            self.computer_hand.append(self.reshuffle_draw())
    ...
    def player_bet(self):
        ...
        if comp_rank >= 1 and self.computer_chips >= bet:
            actual_bet = min(bet, self.computer_chips)
            self.computer_chips -= actual_bet
            self.pot += actual_bet
            self.status_label.config(text=f"Computer calls {bet}.")
            self.update_chips_display()
            if actual_bet < bet: #allows computer to go all in and bet properly with fewer chips
                excess = bet - actual_bet
                self.player_chips += excess
                self.pot -= excess
                self.status_label.config(text=f"Computer went all in! Refunded {excess} chips.")
                self.update_chips_display()
            self.showdown()
        else:
            # folds
...
#corrects running out of cards. reshuffles when running out
